...
### 任务
- [ ] 终端中进入桌面：`cd Desktop`
- [ ] `mkdir test_folder` → `ls` 能看到它
- [ ] `cd test_folder` → `touch hello.txt` → `ls` 能看到 hello.txt
- [ ] `cd ..` → `rm -rf test_folder` → `ls` 确认已删除
- [ ] **理解：** 相对路径（`./file`）vs 绝对路径（`/Users/xxx/Desktop/file`）

### 弹性任务
- [ ] 在桌面建三层嵌套文件夹（mkdir -p a/b/c），然后删掉
...
### 任务
- [ ] 打开浏览器 → 搜索"python.org downloads" → 下载 macOS 版本（推荐3.10或3.11）
- [ ] 双击安装 → 一路"继续"→ 输入 Mac 密码
- [ ] 打开终端，输入 `python3 --version` → 看到 Python 3.x.x → ✅
- [ ] 打开 code.visualstudio.com → 下载安装 VS Code
- [ ] 打开 VS Code → 装中文扩展（搜索 "Chinese"）→ 装 Python 扩展

### 弹性任务
- [ ] 新建文件 `hello.py` → 写 `print("Hello World")` → 终端运行 `python3 hello.py` → 看到输出
...
- [ ] 打开 VS Code，创建 `day11_variables.py`：
  ```python
  name = "小明"
  age = 25
  city = "南京"
